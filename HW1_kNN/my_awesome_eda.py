...
def display_welcome_gif():
    html_code = """
    <img src="pics/welcome.gif" width="200" height="200">
    """
...
def run_eda(df):
    ...
    def print_categorical_counts(df, categorical_features):
        for feature in categorical_features:
            ...
            results = [{'Value': value, 'Counts': count, 'Frequencies': count / total_counts * 100} for value, count in counts.items()]
        
            for row in results:
                print(f"{row['Value']}: Counts={row['Counts']}, Frequencies={row['Frequencies']:.2f}%")
    ...
    def plot_missing_values(df):
        ...
        ax = sns.barplot(x=missing_percentage.index, y=missing_percentage.values, hue=missing_percentage.index, palette='viridis', legend=False)
        ax.set(ylim=(0, 100))
        
        for p in ax.patches:
            ax.annotate(f'{p.get_height():.1f}%', (p.get_x() + p.get_width() / 2., p.get_height()),
                        ha='center', va='center', fontsize=10, color='black', fontweight='bold', xytext=(0, 10),
                        textcoords='offset points')
        
        plt.title('Percentage of Missing Values by Variable', fontsize=16, fontweight='bold', fontstyle='italic')
    ...
    def plot_correlation_heatmap(df, numerical_features):
        ...
        plt.title('Correlation heat map', fontsize=16, fontweight='bold', fontstyle='italic')
    ...
    def plot_numerical_histograms(df, numerical_features):
        for feature in numerical_features:
            ...
            plt.title(f'Boxplot of {feature}', fontsize=12, fontweight='bold', fontstyle='italic')
            plt.xlabel('Count', fontsize=8, fontweight='bold', fontstyle='italic')        

            plt.subplot(2, 1, 2)
            sns.histplot(df[feature], kde=True, color='skyblue')
            plt.title(f'Histogram of {feature}', fontsize=8, fontweight='bold', fontstyle='italic')
